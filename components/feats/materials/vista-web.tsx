...
interface VistaWebProps {
  ...
  categories: string[]
  marcas: any[]
  onUpdateMaterial: (productoId: string, materialCodigo: string, data: any, categoria: string) => Promise<boolean>
}

export function VistaWeb({ materials, categories, marcas, onUpdateMaterial }: VistaWebProps) {
  // Filters
  ...
  // Specs form
  const [editSpecs, setEditSpecs] = useState<{ clave: string; valor: string }[]>([])
  const [savingSpecs, setSavingSpecs] = useState(false)

  // Price form
  const [editPrices, setEditPrices] = useState<{ cantidad: string; precio: string }[]>([])
  ...
  const filteredMaterials = useMemo(() => {
    return materials.filter((material) => {
      ...
      const matchesMarca = selectedMarca === "all" || material.marca_id === selectedMarca
      return matchesSearch && matchesCategory && matchesMarca
    })
  }, [materials, searchTerm, selectedCategory, selectedMarca])
  ...
  const openSpecsDialog = (material: Material) => {
    ...
    setEditSpecs(
      material.especificaciones
        ? Object.entries(material.especificaciones).map(([clave, valor]) => ({ clave, valor }))
        : []
    )
  ...
  const saveSpecs = async () => {
    ...
    setSavingSpecs(true)
    try {
      const especificacionesObj = editSpecs.length > 0
        ? editSpecs.reduce((acc, { clave, valor }) => {
      ...
      const productoId = selectedMaterial.producto_id || selectedMaterial.id.split("_")[0]
      await onUpdateMaterial(
        productoId,
  ...
  const openPriceDialog = (material: Material) => {
    ...
    setEditPrices(
      material.precio_por_cantidad
        ? Object.entries(material.precio_por_cantidad).map(([cantidad, precio]) => ({ cantidad, precio: String(precio) }))
        : []
    )
  ...
  const savePrices = async () => {
    ...
    try {
      const preciosObj = editPrices.length > 0
        ? editPrices.reduce((acc, { cantidad, precio }) => {
      ...
      await onUpdateMaterial(
        ...
        String(selectedMaterial.codigo),
        {
          ...
          potenciaKW: selectedMaterial.potenciaKW,
          habilitar_venta_web: selectedMaterial.habilitar_venta_web,
          precio_por_cantidad: Object.keys(preciosObj || {}).length > 0 ? preciosObj : null,
    ...
    <div className="space-y-6">
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1">
              <Label htmlFor="web-search" className="text-sm font-medium text-gray-700 mb-2 block">
                Buscar Material
              </Label>
            ...
            <div className="lg:w-48">
              <Label htmlFor="web-category-filter" className="text-sm font-medium text-gray-700 mb-2 block">
                Categoría
              </Label>
            ...
            <div className="lg:w-48">
              <Label htmlFor="web-marca-filter" className="text-sm font-medium text-gray-700 mb-2 block">
                Marca
              </Label>
      ...
      {filteredMaterials.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredMaterials.map((material) => {
            ...
            const specsCount = material.especificaciones ? Object.keys(material.especificaciones).length : 0
            const pricesCount = material.precio_por_cantidad ? Object.keys(material.precio_por_cantidad).length : 0

            return (
              <Card
                key={`${material.codigo}-${material.categoria}`}
                className={`border shadow-sm hover:shadow-md transition-shadow ${
                  material.habilitar_venta_web ? "border-l-4 border-l-green-500" : "border-l-4 border-l-gray-300"
                }`}
              >
                <CardContent className="p-4">
                  ...
                  <div className="w-full h-40 rounded-lg overflow-hidden bg-gray-100 mb-3 flex items-center justify-center">
                    ...
                    <div className={`${material.foto ? "hidden" : "flex"} items-center justify-center w-full h-full`}>
                      <Package className="h-12 w-12 text-gray-300" />
                    </div>
                  </div>

                  {/* Name */}
                  <h3 className="font-semibold text-gray-900 text-sm truncate mb-1" title={material.nombre || material.descripcion}>
                  ...
                  <div className="flex items-center justify-between border-t pt-3">
                    ...
                    <button
                      ...
                      title={material.habilitar_venta_web ? "Deshabilitar venta web" : "Habilitar venta web"}
                    >
                      {isToggling ? (
      ...
      <Dialog open={specsDialogOpen} onOpenChange={setSpecsDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Especificaciones - {selectedMaterial?.nombre || selectedMaterial?.descripcion}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {editSpecs.map((spec, index) => (
              <div key={index} className="flex gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => setEditSpecs(editSpecs.filter((_, i) => i !== index))}
                  disabled={savingSpecs}
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => setEditSpecs([...editSpecs, { clave: "", valor: "" }])}
              disabled={savingSpecs}
            >
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              <Button variant="outline" onClick={() => setSpecsDialogOpen(false)} disabled={savingSpecs}>
                Cancelar
              </Button>
              <Button onClick={saveSpecs} disabled={savingSpecs}>
                {savingSpecs ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <Save className="h-4 w-4 mr-1" />}
                Guardar
              </Button>
            </div>
          </div>
      ...
      <Dialog open={priceDialogOpen} onOpenChange={setPriceDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Precio por Cantidad - {selectedMaterial?.nombre || selectedMaterial?.descripcion}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {selectedMaterial?.precio !== undefined && (
              <p className="text-sm text-gray-600">
                Precio base: <strong>${selectedMaterial.precio?.toFixed(2) || "N/A"}</strong>
              </p>
            )}
            {editPrices.map((item, index) => (
              <div key={index} className="flex gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => setEditPrices(editPrices.filter((_, i) => i !== index))}
                  disabled={savingPrices}
                  className="text-red-600 border-red-300 hover:bg-red-50 h-10 w-10 p-0"
                >
                  <X className="h-4 w-4" />
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => setEditPrices([...editPrices, { cantidad: "", precio: "" }])}
              disabled={savingPrices}
            >
              <Plus className="h-4 w-4 mr-1" />
              Agregar
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              <Button variant="outline" onClick={() => setPriceDialogOpen(false)} disabled={savingPrices}>
                Cancelar
              </Button>
