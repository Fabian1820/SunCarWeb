...
export function EditClientDialog({ open, onOpenChange, client, onSubmit, isLoading }: EditClientDialogProps) {
  ...
  const [oferta, setOferta] = useState({
    ...
    elementos_personalizados: client.ofertas?.[0]?.elementos_personalizados || '',
  ...
  useEffect(() => {
    if (open) {
      ...
      if (client.provincia_montaje && provincias.length > 0) {
        const provincia = provincias.find(p => p.nombre === client.provincia_montaje)
        if (provincia) {
  ...
  useEffect(() => {
    if (open && !loadingMateriales && (inversores.length > 0 || baterias.length > 0 || paneles.length > 0)) {
      console.log('🔄 Reseteando oferta con datos del cliente:', client.ofertas?.[0])
      setOferta({
        inversor_codigo: client.ofertas?.[0]?.inversor_codigo || '',
        inversor_cantidad: client.ofertas?.[0]?.inversor_cantidad || 1,
        bateria_codigo: client.ofertas?.[0]?.bateria_codigo || '',
  ...
  }, [open, loadingMateriales, inversores.length, baterias.length, paneles.length, client])
