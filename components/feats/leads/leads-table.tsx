...
export function LeadsTable({
  ...
  const handleUploadFotosLead = async () => {
    if (!leadForUploadFotos || uploadFotoFiles.length === 0 || !onUploadFotos)
      return;
    ...
    try {
      ...
      const resultado = await onUploadFotos(
        ...
        (procesados) => setUploadFotoProgreso(procesados),
      );

      if (
        (resultado?.subidos ?? 0) > 0 &&
        selectedLead?.id &&
        leadForUploadFotos.id === selectedLead.id
      ) {
      ...
      console.error("Error subiendo fotos/videos del lead:", error);
    } finally {
      setUploadingFoto(false);
      setUploadFotoProgreso(0);
    }
  ...
  const generarOpcionesExportacion = useCallback(
    (oferta: OfertaConfeccion) =>
      generarOpcionesExportacionOferta({
        oferta,
        materials,
        marcasMap: construirMarcasMap(marcas),
        lead: leads.find((l) => l.id === oferta.lead_id),
    ...
    [leads, materials, marcas, terminosCondicionesPayloadBTC],
  );
    ...
    <>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Agregar fotos/videos del lead</DialogTitle>
            <DialogDescription>
              {leadForUploadFotos
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="lead-foto-archivo">Archivos</Label>
              <Input
              ...
              <div className="space-y-2">
                ...
                <div className="max-h-48 overflow-y-auto rounded-md border border-gray-200 divide-y divide-gray-100">
                  {uploadFotoFiles.map((file, index) => {
                    ...
                    const procesado =
                      uploadingFoto && index < uploadFotoProgreso;
                    const enCurso =
                      uploadingFoto && index === uploadFotoProgreso;
                ...
                {uploadingFoto && (
                  <p className="text-xs text-violet-700">
                    Subiendo{" "}
                    {Math.min(uploadFotoProgreso + 1, uploadFotoFiles.length)} de{" "}
                    {uploadFotoFiles.length}...
                  </p>
                )}
              </div>
