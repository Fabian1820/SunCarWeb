                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right">Total Materiales Facturados</TableHead>
                                    ...
                                    return (
                                        <React.Fragment key={factura.numero_factura}>
                                            <TableRow className="hover:bg-gray-50">
                                                ...
                                                <TableCell className="capitalize">{factura.mes || 'N/A'}</TableCell>
                                                <TableCell>{factura.fecha || 'N/A'}</TableCell>
                                                <TableCell>
                                                    {factura.cliente_nombre ? (
                                                        factura.cliente_codigo ? (
                                                            <button
                                                                onClick={() => handleClienteClick(factura.cliente_codigo)}
                                                ...
                                                <TableCell className={`text-right font-medium ${factura.total_cobrado_todas_ofertas > 0 ? 'text-green-600' : 'text-gray-600'}`}>
                                                    {formatCurrency(factura.total_cobrado_todas_ofertas)}
                                                </TableCell>
                                                <TableCell className={`text-right font-medium ${factura.monto_pendiente_materiales > 0 ? 'text-red-600' : 'text-gray-600'}`}>
                                                        ...
                                                        <div className="bg-white rounded-lg border border-orange-200 overflow-hidden">
                                                            ...
                                                            <div className="divide-y divide-gray-200">
                                                                {factura.ofertas.map((oferta, idx) => (
                                                                    <div key={oferta.oferta_id} className="px-4 py-3 hover:bg-gray-50 grid grid-cols-12 gap-4 items-center">
                                                                        <div className="col-span-1 text-sm text-gray-500 font-medium">
                                                                            #{idx + 1}
                                                                        </div>
                                                                        <div className="col-span-4">
                                                                            <p className="text-sm font-semibold text-gray-900">{oferta.numero_oferta}</p>
                                                                            <p className="text-xs text-gray-500 mt-0.5">{oferta.nombre_automatico}</p>
                                                                        </div>
                                                                        <div className="col-span-2 text-right">
                                                                            <p className="text-xs text-gray-500">Precio Final</p>
                                                                            <p className="text-sm font-semibold text-gray-900">{formatCurrency(oferta.precio_final)}</p>
                                                                        </div>
                                                                        <div className="col-span-2 text-right">
                                                                            <p className="text-xs text-gray-500">Cobrado</p>
                                                                            <p className={`text-sm font-semibold ${oferta.monto_cobrado > 0 ? 'text-green-600' : 'text-gray-600'}`}>
                                                                                {formatCurrency(oferta.monto_cobrado)}
                                                                            </p>
                                                                        </div>
                                                                        <div className="col-span-2 text-right">
                                                                            <p className="text-xs text-gray-500">Pendiente</p>
                                                                            <p className={`text-sm font-semibold ${oferta.monto_pendiente > 0 ? 'text-red-600' : 'text-gray-600'}`}>
                                                                        ...
                                                                        <div className="col-span-1 text-right">
                                                                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                                                                ...
                                                                                {oferta.monto_pendiente === 0 ? 'Pagada' : 'Pendiente'}
                                                                            </span>
                                                                        </div>
                                                                    </div>
                                                                ))}
