...
export function AveriasTable({
  ...
  const [estadoAveria, setEstadoAveria] = useState<
    "todas" | "pendientes" | "solucionadas"
  >("pendientes");
  ...
  const handleCambiarEstado = async (
    ...
    const nuevoEstado =
      estadoActual === "Pendiente" ? "Solucionada" : "Pendiente";
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Select
                value={estadoAveria}
                onValueChange={(value: any) => setEstadoAveria(value)}
              >
      ...
      <Card className="border-l-4 border-l-red-600">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>
              {estadoAveria === "todas" &&
                `Clientes con Averías (${clients.length})`}
              {estadoAveria === "pendientes" &&
                `Clientes con Averías Pendientes (${clients.length})`}
          ...
          {clients.length === 0 && !loading ? (
            <div className="p-12 text-center">
              ...
              <p className="text-gray-600">
                {estadoAveria === "pendientes" &&
                  "No se encontraron clientes con averías pendientes"}
                {estadoAveria === "solucionadas" &&
                  "No se encontraron clientes con averías solucionadas"}
                {estadoAveria === "todas" &&
                  "No se encontraron clientes con averías"}
              </p>
            </div>
          ) : (
            <>
                    ...
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {(() => {
                            const averiasFiltered = filtrarAverias(
                              client.averias || [],
                            );
                            return averiasFiltered.length > 0 ? (
                              averiasFiltered.map((averia, idx) => (
                                <div
                                  key={averia.id || idx}
                                  className={`border-l-2 ${getBorderColor(averia.estado)} pl-3 pr-2 py-2 bg-gray-50 rounded-r`}
                                >
                              ...
                              <span className="text-xs text-gray-400">
                                Sin averías{" "}
                                {estadoAveria === "todas" ? "" : estadoAveria}
                              </span>
                            );
                          })()}
                        </div>
