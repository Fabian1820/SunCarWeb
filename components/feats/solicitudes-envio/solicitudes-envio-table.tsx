...
function MaterialesThumbs({ s }: { s: SolicitudEnvio }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-0">
      ...
      {extra > 0 && (
        <div className="w-8 h-8 rounded-md border-2 border-white ring-1 ring-slate-200 bg-slate-100 -ml-4 flex items-center justify-center text-[11px] font-medium text-slate-600 shrink-0">
          +{extra}
        </div>
      )}
      <Badge
      ...
      >
        {s.materiales.length} materiales
      </Badge>
    </div>
  );
}
                ...
                <td className="px-3 py-2 text-slate-600 max-w-[12rem]">
                  {s.almacen_id ? (
                    <span className="block truncate">
                      {nombreAlmacen?.(s.almacen_id) ?? s.almacen_id}
                    </span>
                  ) : (
