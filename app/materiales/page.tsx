...
export default function MaterialesPage() {
  ...
  const [editingCategory, setEditingCategory] =
    useState<BackendCatalogoProductos | null>(null);
  const [duplicatesChecked, setDuplicatesChecked] = useState(false);
  const [duplicates, setDuplicates] = useState<
    { codigo: string; materiales: Material[] }[]
  >([]);
  const [isDuplicatesDashboardOpen, setIsDuplicatesDashboardOpen] =
    useState(false);
  ...
  const addMaterial = async (material: Omit<Material, "id">) => {
    ...
    try {
      ...
      if (isNewCategory) {
        ...
        registerNewCategory(categoria, productoId, materialData);
        ...
        if (!productoId) {
          throw new Error(
            "No se pudo determinar el producto para la categoría",
          );
        }

        await addMaterialToProduct(productoId as any, materialData, categoria);
      }

      toast({
        title: "Éxito",
        description: isNewCategory
          ? "Categoría y material creados exitosamente"
  ...
  const updateMaterial = async (
    updatedMaterial: Material | Omit<Material, "id">,
  ) => {
    ...
    try {
      if (categoriaChanged) {
        ...
        if (!productoNuevoId) {
          ...
          console.log(
            "[updateMaterial] Nueva categoría creada con ID:",
            productoNuevoId,
          );
        }
        ...
        console.log(
          "[updateMaterial] Agregando material a categoría:",
          categoria,
        );
  ...
  const updateCategory = async (data: { categoria: string }) => {
    if (!editingCategory) return;

    try {
      const { MaterialService } =
        await import("@/lib/services/feats/materials/material-service");
      await MaterialService.updateCategory(editingCategory.id, {
  ...
  const handleOpenDuplicatesDashboard = useCallback(() => {
    console.log(
      "[handleOpenDuplicatesDashboard] Abriendo dashboard, toastId:",
      duplicateToastIdRef.current,
    );
    setIsDuplicatesDashboardOpen(true);
    // Cerrar el toast si existe
    if (duplicateToastIdRef.current) {
      console.log(
        "[handleOpenDuplicatesDashboard] Cerrando toast:",
        duplicateToastIdRef.current,
      );
  ...
  useEffect(() => {
    if (!loading && materials.length > 0 && !duplicatesChecked) {
      ...
      if (foundDuplicates.length > 0) {
        ...
        const totalDuplicates = foundDuplicates.reduce(
          (sum, dup) => sum + dup.materiales.length,
          0,
        );
        ...
        const toastResult = toast({
          ...
          description: (
            <div className="flex flex-col gap-2">
              <p className="text-sm">
                Se encontraron {foundDuplicates.length} código(s) con duplicados
                ({totalDuplicates} materiales afectados)
              </p>
              <Button
                size="icon"
                variant="outline"
  ...
  console.log(
    "[MaterialesPage] Materials count:",
    materials.length,
    "Filtered count:",
  ...
  const loadMarcas = async () => {
    ...
    try {
      const { MarcaService } =
        await import("@/lib/services/feats/marcas/marca-service");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-yellow-50">
        ...
        actions={
          viewMode === "materials" ? (
            ...
            <Dialog
              ...
              onOpenChange={setIsAddMarcaDialogOpen}
            >
              <DialogTrigger asChild>
                <Button
                  size="icon"
      ...
      <main className="content-with-fixed-header max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 pb-8">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <Button
                    ...
                    className={
                      viewMode === "web" ? "bg-sky-600 hover:bg-sky-700" : ""
                    }
                  >
          ...
          {viewMode === "marcas" ? (
            ...
            <VistaWeb
              ...
              onUpdateMaterial={async (
                ...
                const result = await editMaterialInProduct(
                  productoId,
                  materialCodigo,
                  data,
                  categoria,
        ...
        {/* Edit Marca Dialog */}
        <Dialog
          open={isEditMarcaDialogOpen}
          onOpenChange={setIsEditMarcaDialogOpen}
        >
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
