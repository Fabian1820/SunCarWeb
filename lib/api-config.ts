...
function getApiBaseUrl(): string {
  const fallbackBackendUrl =
    process.env.NODE_ENV === "development"
      ? "http://localhost:8000"
      : "https://api.suncarsrl.com";
  const backendUrlEnv =
    process.env.NEXT_PUBLIC_BACKEND_URL || fallbackBackendUrl;

  if (!process.env.NEXT_PUBLIC_BACKEND_URL) {
    console.warn(
      `⚠️ NEXT_PUBLIC_BACKEND_URL no está definido. Usando fallback: ${fallbackBackendUrl}`,
    );
  }
