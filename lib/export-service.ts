...
export interface ExportOptions {
  ...
  simboloMoneda?: string; // Símbolo/código mostrado junto al monto (ej: $, €, CUP)
  codigoMoneda?: string; // Código ISO para encabezados (ej: USD, EUR, CUP)
...
export async function exportToPDF(options: ExportOptions): Promise<void> {
  ...
  console.log(
    "📄 exportToPDF - Términos y condiciones:",
    options.terminosCondiciones
      ? "SÍ (" + options.terminosCondiciones.length + " caracteres)"
      : "NO",
  );
  ...
  const simboloMoneda = options.simboloMoneda || "$";
  const codigoMoneda = options.codigoMoneda?.trim();
  const encabezadoTotalConMoneda = codigoMoneda
    ? `Total (${codigoMoneda})`
    : "Total";
  ...
  if (componentesPrincipales) {
    ...
    if (componentesPrincipales.inversor) {
      const totalKW = (
        componentesPrincipales.inversor.cantidad *
        componentesPrincipales.inversor.potencia
      ).toFixed(2);
    ...
    if (componentesPrincipales.bateria) {
      const potenciaBateria = (
        componentesPrincipales.bateria.cantidad *
        componentesPrincipales.bateria.capacidad
      ).toFixed(2);
    ...
    if (componentesPrincipales.panel) {
      const totalKW = (
        (componentesPrincipales.panel.cantidad *
          componentesPrincipales.panel.potencia) /
        1000
      ).toFixed(2);
  ...
  if (!options.sinPrecios && !conPreciosCliente) {
    ...
    // Total: 172-198 (26mm)
    doc.text(encabezadoTotalConMoneda, pageWidth - 12, yPosition + 4, {
      align: "right",
    });
  ...
  for (const [seccion, rows] of datosPorSeccion) {
    ...
    for (const row of rows) {
      ...
      if (incluirFotos && fotosMap) {
        const materialCodigo =
          row.material_codigo || row.materialCodigo || row.codigo;
      ...
      if (!sinPrecios && !conPreciosCliente) {
        ...
        doc.text(formatearMonto(precioUnit), 105, camposYPosition, {
          align: "center",
        });
        ...
        doc.text(cantidad.toString(), 120, camposYPosition, {
          align: "center",
        });
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(0, 0, 0);
        doc.text(formatearMonto(totalBase), 140, camposYPosition, {
          align: "center",
        });
        ...
        if (margen > 0) {
          ...
          doc.setFont("helvetica", "bold");
          doc.setTextColor(0, 0, 0);
          doc.text(formatearMonto(margen), 165, camposYPosition, {
            align: "center",
          });
        } else {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(0, 0, 0);
        doc.text(formatearMonto(totalFinal), pageWidth - 12, camposYPosition, {
          align: "right",
        });
      } else if (conPreciosCliente) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(0, 0, 0);
        doc.text(formatearMonto(total), pageWidth - 12, camposYPosition, {
          align: "right",
        });
      } else {
        ...
        doc.setTextColor(0, 0, 0);
        const cantidad = row.cantidad || "";
        doc.text(cantidad.toString(), pageWidth - 12, camposYPosition, {
          align: "right",
        });
      }
    ...
    if (!sinPrecios && rows.length > 1) {
      ...
      doc.text(formatearMonto(subtotalSeccion), pageWidth - 12, yPosition + 5, {
        align: "right",
      });
  ...
  if (
    ...
    const seccionesCosto = options.seccionesPersonalizadas.filter(
      (s) => s.tipo_extra === "costo",
    );

    for (const seccion of seccionesCosto) {
      if (!seccion.costos_extras || seccion.costos_extras.length === 0)
        continue;
      ...
      for (const costo of seccion.costos_extras) {
        ...
        if (!sinPrecios && !conPreciosCliente) {
          ...
          doc.setFont("helvetica", "bold");
          doc.setTextColor(0, 0, 0);
          doc.text(costo.cantidad.toString(), 120, camposYPosition, {
            align: "center",
          });
          ...
          doc.setFont("helvetica", "bold");
          doc.setTextColor(0, 0, 0);
          doc.text(formatearMonto(totalBase), pageWidth - 12, camposYPosition, {
            align: "right",
          });
        } else if (conPreciosCliente) {
          // Con precios cliente
          ...
          doc.setFont("helvetica", "bold");
          doc.setTextColor(0, 0, 0);
          doc.text(costo.cantidad.toString(), 161, camposYPosition, {
            align: "right",
          });
          ...
          doc.setFont("helvetica", "bold");
          doc.setTextColor(0, 0, 0);
          doc.text(costo.cantidad.toString(), pageWidth - 12, camposYPosition, {
            align: "right",
          });
        }
  ...
  if (
    ...
    const seccionesTexto = options.seccionesPersonalizadas.filter(
      (s) => s.tipo_extra === "escritura",
    );
    ...
    for (const seccion of seccionesTexto) {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(0, 0, 0);
      doc.text(seccion.label.toUpperCase(), pageWidth / 2, yPosition, {
        align: "center",
      });
  ...
  if (datosPago.length > 0 && tienePagoTransferencia) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 0, 0);
    doc.text("DETALLES DEL PAGO", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 5; // Reducido de 8 a 5 para estar más pegado
    ...
    if (false && tieneContribucion && montoContribucion) {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(`$${montoContribucion}`, pageWidth - margenDer, yPosition, {
        align: "right",
      });
      yPosition += 8;
    }
    ...
    if (tieneConversion && monedaPago) {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(monedaPago, pageWidth - margenDer, yPosition, {
        align: "right",
      });
      yPosition += 6;
  ...
  if (options.terminosCondiciones) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 0, 0);
    doc.text("TÉRMINOS Y CONDICIONES", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 5; // Reducido de 8 a 5 para estar más pegado
  ...
  if (typeof document !== "undefined") {
    ...
    const processNode = (node: Node): string => {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (
          ["h1", "h2", "h3", "h4", "h5", "h6", "p", "div"].includes(tagName)
        ) {
          result += "\n";
        }
