...
interface ComprobanteData {
  ...
  monto_pendiente_despues_pago?: number; // Monto pendiente exacto luego de aplicar este pago
}
...
export class ExportComprobanteService {
  ...
  private static dibujarComprobante(
    ...
    doc.text(`${this.formatearMoneda(oferta.precio_final)} USD`, margenDer, y, {
      align: "right",
    });
    y += 5;
    ...
    if (
      ...
      data.total_pagado_anteriormente > 0
    ) {
      doc.text("Monto Pagado Anteriormente:", margenIzq, y);
      doc.text(
        `${this.formatearMoneda(data.total_pagado_anteriormente)} USD`,
        margenDer,
        y,
        { align: "right" },
      );
    ...
    doc.text(
      `${this.formatearMoneda(pago.monto)} ${pago.moneda}`,
      margenDer,
      y,
      { align: "right" },
    );
    ...
    if (pago.moneda !== "USD") {
      ...
      doc.text(
        `1 ${pago.moneda} = ${pago.tasa_cambio.toFixed(4)} USD`,
        margenDer,
        y,
        { align: "right" },
      );
      ...
      doc.text(`${this.formatearMoneda(pago.monto_usd)} USD`, margenDer, y, {
        align: "right",
      });
      y += 5;
    }
    ...
    doc.text(this.formatearMetodoPago(pago.metodo_pago), margenDer, y, {
      align: "right",
    });
    y += 5;
    ...
    // Prioridad: usar el pendiente exacto calculado por la vista al momento de exportar.
    let montoPendiente = data.monto_pendiente_despues_pago;
    ...
    if (
      ...
      const diferencia =
        pago.diferencia && pago.diferencia.monto > 0
    ...
    doc.text(`${this.formatearMoneda(montoPendiente)} USD`, margenDer, y, {
      align: "right",
    });
    y += 8;
