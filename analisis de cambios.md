...
- **`refactor(consignaciones): quitar botón "Nueva Consignación" del módulo`** (Jun 10, 14:20) — Las consignaciones solo se crean desde Pagos con el toggle. Se elimina el botón que pedía ObjectId directamente.
...
- **`fix(consignaciones): toggle no crea pago — solo crea consignación`** (Jun 10, 18:06) — **Cambio de flujo crítico**: el flujo anterior creaba un `PagoVenta` Y una `Consignacion`, dejando la solicitud aparentemente pagada al 100%. Nuevo flujo: el toggle solo llama a `ConsignacionService.crear({solicitud_venta_id, moneda})` sin crear pago; los cobros posteriores se registran como pagos normales que la auto-vinculación asocia al saldo de la consignación.
- **`ui(consignaciones): mover toggle al inicio del dialog de pago`** (Jun 10, 18:17) — El toggle pasa a estar arriba del formulario. Al activarse, oculta todos los campos de cobro (monto, método, billetes, plazos).
- **`fix(consignaciones): UX — sin ObjectIds, step entero, listar pagos reales`** (Jun 11, 14:42) — `VincularPagoDialog`: eliminado input manual de ObjectId; ahora lista los `PagoVenta` existentes de la solicitud con monto/método/fecha/recibido_por. `RegistrarDevolucionDialog`: step=1, texto en lenguaje humano. Tabla y detalle: tipo `Consignacion` trae `solicitud_codigo`, `cliente_nombre`, `almacen_nombre` y `pagos[]` denormalizados del backend.
...
- **`fix(rrhh): evitar creación duplicada de empleado por doble-clic`** (Jun 11, 17:55) — El formulario no recibía `isSubmitting` del padre y el botón permanecía habilitado durante el POST. Se agrega guarda interna que bloquea envíos concurrentes y deshabilita controles apenas inicia el submit.

---

### Área 3: Facturas solar-carros — edición + carga perezosa (1 commit — yany1509, Jun 11 17:09)

- **`feat(facturas-solar-carros): botón editar + carga perezosa por pestaña`** — Botón "Editar" en facturas con dialog que hace `PATCH /facturas-solar-carros/{id}` (campos: no, fecha, cliente, concepto, moneda, monto). Carga perezosa por pestaña (instaladora/ventas/facturas) — antes la pestaña Ventas pedía 2000 clientes + 2000 solicitudes al montar. Spinners independientes por pestaña. `pageSize` de facturas sube de 200 a 500.
...
2. **Sub-permiso implícito fix — usuarios con padre sin hijo en BD**: El fix exige que `x/y` esté explícitamente en `modulosPermitidos`. Usuarios que solo tenían `x` asignado antes del fix (y dependían del comportamiento implícito) ahora perderán acceso a las funciones de `x/y`. Revisar qué usuarios necesitan re-asignación.

3. **`fix(consignaciones): toggle no crea pago` — auto-vinculación no confirmada**: El nuevo flujo depende de que el backend auto-asocie los pagos normales posteriores al saldo de la consignación. Si la auto-vinculación no está implementada o falla, los pagos quedarán desvinculados y el saldo de la consignación nunca se reducirá.

4. **`VincularPagoDialog` lista pagos reales — endpoint no confirmado**: El dialog lista `PagoVenta` existentes de la solicitud. Confirmar que el backend devuelve los campos `monto`, `metodo`, `fecha`, `recibido_por` en el endpoint consultado; si no, la lista aparecerá vacía o con `undefined`.

5. **Consignaciones denormalizadas — campos del backend**: La tabla y el detalle esperan `solicitud_codigo`, `cliente_nombre`, `almacen_nombre` y `pagos[]` en la respuesta del listado de consignaciones. Si el backend no los devuelve, la UI mostrará vacíos o fallback a fragmentos de ID.

6. **`PATCH /facturas-solar-carros/{id}` — confirmar endpoint**: El botón Editar asume que este endpoint existe y acepta los campos editados. Si no existe, la edición devolverá 404 sin aviso claro.

7. **Fix doble-clic RRHH — sin mecanismo de retry tras error de red**: La guarda interna bloquea envíos concurrentes. Si la primera petición falla por error de red antes de llegar al servidor (timeout, CORS), el guard queda activo y el usuario necesita recargar la página para intentar de nuevo.

8. **Carga perezosa por pestaña — datos no actualizados al volver**: Los datos se cargan una sola vez al visitar la pestaña. Si el usuario modifica datos en otro módulo y vuelve a una pestaña ya cargada, los datos pueden estar desactualizados. No hay refresh automático al cambiar de pestaña.

9. **Refactor RRHH de 1410 a 579 líneas — posible pérdida de funcionalidad**: Una reducción tan grande en una sola pasada puede haber eliminado casos edge o validaciones no evidentes. Verificar flujos completos (crear, editar, desactivar, permisos).
...
- **`precio → costo` rename en Asignacion — fallback solo en service**: Confirmar que ningún componente accede a `asignacion.precio` directamente.
- **Endpoints de ajuste de costo y transferencia de asignaciones**: Confirmar que el backend implementa endpoints específicos para `AjustarCosto` y `TransferirAsignacion`.
- **`PlanDepreciacionView` — campos calculados en endpoint**: Confirmar que el backend devuelve `valor_depreciado` y `valor_residual`.
...
- **Tipo `reserva_pendiente` en notificaciones**: Confirmar que el backend genera notificaciones con ese tipo exacto.
...
### Área 1: Rebrand Suncar 2026 (yany1509)

Nueva paleta verde (Emerald Circuit, Volt Green, Solar Radiance, Midnight Voltage, Clean Current). Centralizada en `globals.css` y `tailwind.config.ts`. Barrido global naranja → emerald. Tema Ventas (`[data-area=ventas]`) con navy+amarillo. Rebrand de exportaciones de ofertas.

---

### Área 2: Dashboard rediseñado (Ruben0304)

Avatar de trabajador. Widget de clima horario (Open-Meteo, La Habana, sin clave), carrusel de cumpleaños de la semana, contador de instalaciones solares. Eliminadas cards de módulos y favoritos. Widget de paneles simplificado a estado único.

---

### Área 3: Navegación — botón atrás por área (Ruben0304)
...
Iconos regenerados con logo suncar-v2-iso. Icono verde en modo oscuro.

---

### Área 5: Exportación Excel refactorizada (Fabian1820)

Primera versión: columnas dinámicas. Segunda versión: apilado vertical con merge de celdas para facturas, solicitudes y vales de salida.

---

### Área 6: Transferencias — botón Resolver (Fabian1820)
...
1. **Widget de clima — 3 patches en cascada (16:34→16:36)**: `AbortController` puede comportarse diferente en producción sin StrictMode.
2. **`window.history.pushState` + Next.js App Router desync**: El `popstate` listener puede no dispararse en el primer render SSR.
3. **Export Excel — 2 refactors en 1.5h**: Merge vertical puede ser incompatible con Excel 2010/2013.
4. **Rebrand masivo**: Componentes con clases `orange-*` directas mostrarán colores incorrectos.
5. **Widget de paneles — estado único sin contrato de API confirmado**.
6. **`POST /solicitudes-transferencia/{id}/resolver` — endpoint pendiente**.
...
- **Sort client-side de solicitudes pendientes en ValesSalida**.
- **Parsing UTC→local en otras tablas con filtros de fecha**.
- **Tasas MLC/CUP sin persistencia entre sesiones (nuevo)**.
- **`PonderarCostoResponse` campos nuevos (nuevo)**.
- **`GET /api/kardex-costo/costo-actual` (nuevo)**.
- **`materiales` en respuesta de facturas de solicitudes-ventas (nuevo)**.
- **Filtros de vales de salida — `fecha_desde`, `fecha_hasta`, creador (nuevo)**.
- **`almacenes-suncar/admin` — gating solo en frontend (nuevo)**.
- **Estados de transferencia no mapeados en `ESTADO_CONFIG` (nuevo)**.
- **Campos de dimensionamiento en calculadora sin persistencia confirmada (nuevo)**.
- **Badges de disponibilidad por pool — snapshot estático (nuevo)**.
- **Endpoint cumpleaños de la semana (nuevo)**.
- **Endpoint contador de instalaciones solares (nuevo)**.
- **Widget de paneles — estado único vs respuesta del backend (nuevo)**.
- **`window.history.pushState` + Next.js App Router desync (nuevo)**.
- **Export Excel merge vertical — heterogeneidad de materiales (nuevo)**.
- **Rebrand paleta — componentes con clases hardcoded (nuevo)**.
- **`POST /solicitudes-transferencia/{id}/resolver` — endpoint pendiente de confirmación (nuevo)**.

---

> ⚠️ **Nota de mantenimiento**: Las entradas del **26, 27, 28, 29, 30 de Mayo, 31 de Mayo, 1 y 2 de Junio** fueron eliminadas al superar los 7 días de antigüedad (política de retención semanal).
