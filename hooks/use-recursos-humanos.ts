...
export function useRecursosHumanos() {
  ...
  const [ultimoIngreso, setUltimoIngreso] = useState<IngresoMensual | null>(
    null,
  );
  ...
  const loadAsistencia = useCallback(
    async (trabajadoresActuales: TrabajadorRRHH[]) => {
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          err.message?.includes("validación")
        ) {
  ...
  const actualizarDiasNoTrabajados = useCallback(
    async (
      ci: string,
      dias: number[],
    ): Promise<{ success: boolean; message: string }> => {
  ...
  const actualizarTrabajador = useCallback(
    async (
      ...
      try {
        ...
        return {
          success: false,
          message: err.message || "Error al actualizar el trabajador",
        };
      }
    },
    [],
  ...
  const guardarIngresoMensual = useCallback(
    async (
      monto: number,
      mes: number,
      anio: number,
      ...
      try {
        ...
        if (ingresoExistente?.id) {
          ...
          return {
            success: true,
            message: "Ingreso mensual creado correctamente",
          };
        }
      } catch (err: any) {
        ...
        return {
          success: false,
          message: err.message || "Error al guardar el ingreso mensual",
        };
      }
    },
    [ultimoIngreso],
    ...
    async (
      ...
      try {
        ...
        if (data.porcentaje_fijo_estimulo !== undefined)
          rrhhData.porcentaje_fijo_estimulo = data.porcentaje_fijo_estimulo;
        if (data.porcentaje_variable_estimulo !== undefined)
          rrhhData.porcentaje_variable_estimulo =
            data.porcentaje_variable_estimulo;
        if (data.alimentacion !== undefined)
          rrhhData.alimentacion = data.alimentacion;
        if (data.dias_trabajables !== undefined)
        ...
        if (Object.keys(rrhhData).length > 0) {
          ...
          await RecursosHumanosService.actualizarTrabajadorRRHH(
            data.ci,
            rrhhData,
          );
        }
